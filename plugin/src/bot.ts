...
export function createTelegramBot(
  ...
  bot.on("message:text", async (ctx) => {
    ...
    // Send "Prompt:" message and store the message ID
    ...
    try {
      ...
      // Note: We'll track the response message ID when we receive the message.updated event
      // The prompt response doesn't directly contain the message ID we need
      // We store the prompt message ID in session store so we can link it later
